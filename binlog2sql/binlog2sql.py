            ...
            for binlog_event in stream:
                if not self.stop_never:
                    try:
                        ...
                        event_time = datetime.datetime(1980, 1, 1, 0, 0)
                    if (stream.log_file == self.end_file
                            and stream.log_pos == self.end_pos) or (
                                stream.log_file == self.eof_file
                                and stream.log_pos == self.eof_pos):
                        flag_last_event = True
                        flag_last_event = True
                    elif event_time < self.start_time:
                        if not (isinstance(binlog_event, RotateEvent)
                        ...
                        continue
                            ...
                            self.end_pos and stream.log_file == self.end_file
                            and stream.log_pos > self.end_pos) or (
                                stream.log_file == self.eof_file
                                and stream.log_pos > self.eof_pos) or (
                                    event_time >= self.stop_time):
                        break
                    # else:
                    #     raise ValueError('unknown binlog file or position')
